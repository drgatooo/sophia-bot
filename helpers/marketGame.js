...
class MarketGame{
  ...
  generateArray(){ //Genera el array (se debe utilizar justo despues de instanciar)
    for(let i = 0; i < this.rows; i++){
      this.board[i] = []
      for(let j = 0; j < this.columns; j++){
        this.board[i][j] = spaceChar
      }
    }
    return this.board
  }

  checkCollectorPosition(){ //Boolean, Verifica la posicion del collector (carrito)
    ...
    if(this.position !== emojiIndex && this.emojis.trash.includes(row[emojiIndex])) return true
  ...
  updateFrame(){ //Array, actualiza el frame de los emojis 
    this.moveEmojis()
    this.generateEmoji()
   ...
   nextEmojiInfo(){
    ...
    return {trash: this.emojis.trash.includes(emoji), food: this.emojis.food.includes(emoji)}
  }
  ...
  ascii(){ //String, retorna una string para verse desde terminal, lo cambias segun lo que quieras poner
    let str = ''
    ...
    str += '\n'
    const collectorRow = [...Array(this.columns)].map((_, i) => i === this.position ? this.collector.unicode : '*')
    str += collectorRow.join(' ')
    return str
  }

  moveLeft(){ //Number, mueve el collector a la izquierda
    if(this.position > 0) this.position--
    return this.position
  }

  moveRight(){ //Number, mueve el collector a la derecha
