    ...
    if (ch && ch.ChannelID) {
        try {
            ...
            if(components){
                ...
                welcome.setImage(`https://api.popcat.xyz/welcomecard?background=${components.Imagen}&text1=${member.user.username}&text2=Bienvenido+a+${member.guild.name}&text3=Pasalo+bien!&avatar=${member.user.displayAvatarURL({ format: "png", dinamyc: false })}`.trim().split(/ +/).join("+"))
            } else {
                ...
                welcome.setImage(`https://api.popcat.xyz/welcomecard?background=${member.guild.iconURL({ dynamic: false, format: "png" })}&text1=${member.user.username}&text2=Bienvenido+a+${member.guild.name}&text3=Pasalo+bien!&avatar=${member.user.displayAvatarURL({ format: "png", dinamyc: false })}`.trim().split(/ +/).join("+"))
            }
