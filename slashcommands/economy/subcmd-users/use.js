...
const command = {
	...
	data: new SlashCommandSubcommandBuilder()
		...
		.addIntegerOption((o) =>
			o
				.setName('numero')
				.setDescription('Número del objeto en el inventario.')
				.setRequired(true),
		),
				...
				if (results) {
					...
					try {
						if (results && results.inventory[number].rolId !== null) {
							...
							const result = results.inventory[number].rolId

							author.roles.add(result).catch(async () => {
								err.setDescription(
									'No he podido agregarte el rol, [puede ser por falta de permisos]\nConsulta con un administrador para resolver el problema!',
								)
							...
							if (!author.roles.cache.has(result)) {
								exi.setDescription(
									'Rol <@&' +
										results.inventory[number].rolId +
							...
							exi.setDescription(
								'Producto **' +
									results.inventory[number].product +
									'** usado correctamente.',
							)
							await interaction.reply({ embeds: [exi] })
							results.inventory.splice(number, 1)
						...
						return await interaction.reply({
							embeds: [
								new MessageEmbed()
									.setTitle(':x: Error')
									.setDescription(
										'No se ha podido agregar el rol, revisa que el número que hayas puesto exista en tu inventario.',
									)
									.setColor('RED'),
							],
							ephemeral: true,
