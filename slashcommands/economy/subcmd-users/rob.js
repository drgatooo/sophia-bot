...
const command = {
	...
	data: new SlashCommandSubcommandBuilder()
		...
		.addUserOption((o) =>
			o.setName('usuario').setDescription('Usuario a robar!').setRequired(true),
		),
	...
	async run(client, interaction) {
		...
		if (userCooldowned) {
			...
			if (timeLeft.hours >= 1) error.addField('Tiempo restante:', `${timeLeft.hours} horas`, true)
			...
			const embedSuccess = new MessageEmbed()
				.setDescription(
					`**${interaction.user.username}** ha robado \`${randomMoney}$\` ha **${mention.username}**`,
				)
				.setColor('GREEN')
				.setFooter({
					text: `${mention.username} asegurate de siempre guardarlo en el banco...`,
				})
				.setTimestamp()
