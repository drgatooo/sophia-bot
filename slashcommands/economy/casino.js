...
const command = {
	...
	async run(client, interaction) {
		if (interaction.options.getSubcommand() === 'carrera-caballos') {
			const money = interaction.options.getInteger('apuesta')
			const balance = await economyModel.findOne({ guildid: interaction.guild.id, userid: interaction.user.id })
			...
			function updateTable() {
            	...
            	players[choice].position[1] += 1
						if (players[choice].position[1] > table[players[choice].position[0]].length - 1)
							players[choice].position[1] = table[players[choice].position[0]].length - 1
						break
					case 2:
						table[players[choice].position[0]][players[choice].position[1]] = '*'
			...
			function drawEmojiInTable() {
				const choices = Object.keys(players)
				choices.forEach(choice => {
					table[players[choice].position[0]][players[choice].position[1]] = horseEmoji
				})
			}
			...
			function createAsciiMessage() {
				const embed = new MessageEmbed()
					...
					.setTimestamp()
				embed.setDescription('```' + updateFrame().map(row => row.join(' ')).join('```\n```') + '```')
				return [embed]
			}
			...
			const interval = setInterval(async () => {
				...
				const winner = Object.keys(players).find(choice => players[choice].position[1] === 9)
				if (winner) {
					winners.push(winner)
					...
					await interaction.editReply({ embeds: [
						new MessageEmbed()
							...
							.setTitle('Carrera de caballos')
							.setTimestamp()
							.setDescription(`${winner === horseSelected ? '**¡Has ganado!**' : '**Has perdido.**'}\nEl ganador es el caballo número \`${winner}\``),
					] })

					economyModel.findOne(
						{ guildid: interaction.guildId, userid: interaction.user.id }, (err, result) => {
							if (err) throw err
							if (result) {
								if (winner === horseSelected)
									result.money += money
