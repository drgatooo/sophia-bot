...
const command = {
    ...
    async run(_, interaction){
        ...
        if(args.getString('lugar') === "bank"){
            const resultsBank = await schemaBank.findOne({guildid: interaction.guild.id, userid: mention.user.id});
            ...
            if(resultsBank) {
                ...
                exi.setDescription(`:atm: Se le cargaron \`${Intl.NumberFormat().format(cantidad)}$\` en el banco a <@${mention.user.id}>`)
                ...
                await newBank.save();
                exi.setDescription(`💵 Le has dado \`${Intl.NumberFormat().format(cantidad)}$\` en efectivo a <@${mention.user.id}>`)
                await interaction.reply({embeds: [exi]});
            }
            return;
        }
    
        const resultsWallet = await schema.findOne({guildid: interaction.guild.id, userid: mention.user.id});
