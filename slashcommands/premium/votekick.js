...
const command = {
	...
	data: new SlashCommandBuilder()
		...
		.addUserOption((o) =>
			o
				.setName('usuario')
				.setDescription('El usuario que deseas enviar a la votación.')
				.setRequired(true),
		),
	...
	async run(client, interaction) {
		...
		const embed = new MessageEmbed()
			...
			.setDescription(
				'Se ha abierto una nueva votación para sacar a un usuario del discord, participa en la decisión votando!',
			)
			...
			return interaction.reply({
				embeds: [
					embedError.setDescription(
						'El miembro tiene un rol superior al tuyo.',
					),
				],
				ephemeral: true,
			})
		...
		if (!kickMember.kickable) {
			return interaction.reply({
				embeds: [embedError.setDescription('El miembro no puede ser expulsado')],
				ephemeral: true,
			})
		}

		interaction.reply({
			embeds: [new MessageEmbed().setTitle('Listo!').setColor('GREEN')],
			ephemeral: true,
		})
		await interaction.channel.send({ embeds: [embed] }).then(async (msg) => {
			...
			const filtro = (reaction) =>
				reaction.emoji.name === kickear || reaction.emoji.name === noKickear
			...
			if (SI_Cuenta == undefined) var SI_Cuenta = 1
			else var SI_Cuenta = reacciones.get(kickear).count

			if (NO_Cuenta == undefined) var NO_Cuenta = 1
			else var NO_Cuenta = reacciones.get(noKickear).count
					...
					msg.edit({
						embeds: [
							new MessageEmbed()
								.setColor('RED')
								.setDescription(
									`${kickuser} Ha sido Kickeado Exitosamente`,
								),
						],
					})
				})
