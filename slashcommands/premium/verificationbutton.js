		...
		if (subcmd === 'enable') {
			if (existe) {
				await interaction.reply({
					embeds: [
						new MessageEmbed()
							.setTitle(':x: Error')
							.setDescription(
								`El sistema ya se encuentra activo en <#${existe.ChannelID}>`,
							)
							.setColor('RED'),
					],
					ephemeral: true,
				...
				if (channel) {
					...
					if (existe && existe.ChannelID === channel.id) {
						return await interaction.reply({
							embeds: [sameID],
							ephemeral: true,
						})
				...
				if (rol) {
					...
					if (rol.tags) {
						return await interaction.reply({
							embeds: [
								err.setDescription('No puede ser el rol de un bot!'),
							],
							ephemeral: true,
						})
					}
				...
				const respuesta = new MessageEmbed()
					...
					.setDescription(
						'Acabo en enviar la verificación al canal seleccionado, para poder establecer otro canal usa `/verificationbutton disable`',
					)
					.setColor('GREEN')
