...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('viewconfig')
		.setDescription(
			'Revisa la configuración de tu servidor de los diversos sistemas de Sophia.',
		),

	async run(client, interaction) {
		// Models
		const welcomeChannel = await welcomeModel.findOne({
			ServerID: interaction.guild.id,
		})
		const leaveChannel = await leaveModel.findOne({ ServerID: interaction.guild.id })
		const leaveComponents = await componentslLeave.findOne({
			ServerID: interaction.guild.id,
		})
		const invitesChannel = await inviteModel.findOne({
			ServerID: interaction.guild.id,
		})
		const components = await componentsModel.findOne({
			ServerID: interaction.guild.id,
		})
		...
		const checked = '<a:Stable:910938393968517180>'
		const unchecked = '<a:Down:910938393993699350>'
		const gears = '<:Employee:910880101758029874>'
		...
		const configview = new MessageEmbed()
			...
			.addField(
				'🔍 Anti-Links',
				`${
					Antilinks ? `${checked} **Activado**` : `${unchecked} **Desactivado**`
				}`,
				true,
			)
			.addField(
				'📴 Anti-Spam',
				`${
					AntiSpam ? `${checked} **Activado**` : `${unchecked} **Desactivado**`
				}`,
				true,
			)
			...
			.setFooter({
				text: `${
					premium
						? '🚀 El servidor es premium!'
						: '❌ El servidor no es premium'
				}`,
