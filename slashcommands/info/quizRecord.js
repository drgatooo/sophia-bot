...
module.exports = {
	...
	async run(client, interaction) {
		...
		const embedTiempo = new MessageEmbed().setColor('ORANGE')
		if (usuario) embedTiempo.setTitle(`🎲 Estadisticas QuizWords de ${usuario.tag}`)
		else embedTiempo.setTitle('🎲 Estadisticas de QuizWords')

		embedTiempo.addField(
			'⌛ Tiempo Record:',
			`${
				quizTimer.TimeMax
					? `\`${quizTimer.TimeMax}\` segundos!`
					: '`No hay ningun record`'
			}`,
		)
		embedTiempo.addField(
			'🔥 Racha:',
			`${
				quizTimer.Racha
					? `Tienes una racha de \`${quizTimer.Racha}\` palabras seguidas!`
					: '`0`'
			}`,
		)
		embedTiempo.addField(
			'🔠 Palabra mas larga descubierta:',
			`${quizTimer.LongWord ? `\`${quizTimer.LongWord}\`` : '`ninguna`'}`,
		)
		embedTiempo.addField(
		...
		embedTiempo.addField(
			'❎ Cantidad de palabras no acertadas:',
