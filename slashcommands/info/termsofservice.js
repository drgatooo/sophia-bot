...
const command = {
    ...
    async run(client, interaction){
        ...
        collector.on("collect", async i => {
            ...
            if(i.customId === "text"){
                interaction.editReply({embeds: [embed.setDescription("Okay 🧡,\nprocedo a borrar el mensaje en 30 segundos, has elegido la opción ephemeral.")], components: []}).then(() => {
                    setTimeout(() => {
                        interaction.deleteReply()
                    }, 30000)
                })
                interaction.followUp({embeds: [embed.setDescription(tos), toc], ephemeral: true})
            }
        })
    }
}
