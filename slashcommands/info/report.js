			...
			o
				...
				.addStringOption((option) =>
					option
						.setName('bug')
						.setRequired(true)
						.setDescription('El bug en cuestión'),
				)
				.addStringOption((option) =>
					option
						.setName('imagen')
						.setDescription('¿Tienes algun link de imagen? ¡Compartenosla!'),
				),
		)
		.addSubcommand((o) =>
			o
				.setName('user')
				.setDescription('Reporta el mal uso del bot de un usuario.')
				.addStringOption((option) =>
					option
						.setName('tag')
						.setRequired(true)
				...
				.addStringOption((option) =>
					option
						.setName('reporte')
						.setRequired(true)
						.setDescription('Motivo del porque lo reportas.'),
				)
				.addStringOption((option) =>
					option
						.setName('imagen')
	...
	async run(client, interaction) {
		...
		if (subcmd === 'bug') {
			...
			const embed = new MessageEmbed()
				...
				.setDescription(
					'Estás a punto de reportar un bug, lo cual conlleva que si es algo inapropiado, puedas ser sancionado. ¿Deseas proceder?',
				)
				.addField(
					'Tu reporte:',
		...
		if (subcmd === 'user') {
			...
			const embed = new MessageEmbed()
				.setTitle('<a:attention:910938394861916160> Panel de confirmación')
				.setDescription(
					'Estás a punto de reportar a un usuario, lo cual conlleva que si es un reporte invalido, puedas ser sancionado. ¿Deseas proceder?',
				)
				.addField(
					'Tu reporte:',
