...
const command = {
    ...
    async run(client, interaction){
        ...
        if(usuario.id === interaction.user.id) return interaction.reply({ 
            embeds: [ 
                embed.setDescription("No te puedes auto mutear.")
            ],
            ephemeral: true 
        })
        if(usuario.user.bot) return interaction.reply({
            embeds: [ 
                embed.setDescription("No puedes mutear a un bot.") 
            ],
            ephemeral: true 
        })
        if(usuario.isCommunicationDisabled()) return interaction.reply({
            embeds: [
                embed.setDescription(`El usuario **${usuario.user.username}** ya se encuentra en un hard mute`)
            ],
            ephemeral: true
        })
        ...
        if(time < 59000) return interaction.reply({embeds: [ 
        ...
        interaction.reply({
            embeds: [
                ...
                .setDescription(`Que mala suerte, al parecer el usuario ${usuario} ha sido hard muteado (aislado) del resto de personas, el actualmente ve los mensajes pero no puede escribir, nos vemos luego amig@.`)
        ...
        usuario.send({
            embeds: [
                ...
                .setDescription(`Que mala suerte, ${usuario} has sido hard muteado (aislado) del resto de personas en ${interaction.guild.name}, nos vemos luego amig@.`)
                .addField("Usuario:", `${usuario}`, true)
                .addField("Tiempo:", `${tiempo} minuto(s)`, true)
                .addField("Razón:", `${razon}`, true)
                .setColor("#00FFFF")
