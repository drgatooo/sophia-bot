...
const command = {
    ...
    if(interaction.user.roles.highest.comparePositionTo(user.roles.highest) <= 0) return interaction.reply({embeds: [noPosible.setDescription('ℹ No puedes warnear a alguien con mayor o igual rol que tu.')], ephemeral: true});
    
        const guildId = interaction.guild.id;
        const userId = user.id;
        let reason = interaction.options.getString("razon")
        if(reason.length > 50) return message.reply({embeds: [noPosible.setDescription('ℹ Solo puedes poner hasta 50 caracteres!')], ephemeral: true});
