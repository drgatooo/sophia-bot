...
const command = {
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((o) =>
			o.setName('razón').setDescription('Razón de la expulsion').setRequired(false),
		),
	...
	async run(client, interaction) {
		...
		const user = interaction.guild.members.cache.get(args.getUser('usuario').id)
		let reason = args.getString('razón')

		if (user.id === interaction.user.id) {
			const noKickYourself = new MessageEmbed()
				.setTitle('❌ Error')
		...
		if (user.id === client.user.id) {
			const noKickBot = new MessageEmbed()
				.setTitle('❌ Error')
		...
		if (!user.kickable) {
			const noKickeable = new MessageEmbed()
				.setTitle('❌ Error')
				.setDescription(
		...
		if (reason !== 'sin especificar') success.addField('💥 Razón: ', reason, true)
				...
				await interaction.reply({
					embeds: [
						new MessageEmbed()
							.setTitle(':x: Error')
							.setDescription(
								'No se pudo kickear al usuario por un error interno, ya fue avisado a mis creadores para solucionarlo.',
							)
							.setColor('RED'),
					],
					ephemeral: true,
