					...
					o
						...
						.addStringOption((o) =>
							o
								.setName('razon')
								.setDescription(
									'Razon del porque añades este servidor a la blacklist.',
								),
						)
						.addStringOption((o) =>
							o
					...
					o
						...
						.addStringOption((o) =>
							o
								.setName('razon')
								.setDescription(
									'Razon del porque añades este usuario a la blacklist.',
								),
						)
						.addStringOption((o) =>
							o
		...
		if (subgroup === 'user') {
			if (subcmd === 'add') {
				...
				if (!hasBlacklist) {
					...
					const expireUser = tiempo
						? `<t:${hasBlacklist.expire}:R>`
						: 'No especificado'
					...
					const { Reason, ModeratorID } = hasBlacklist
					const moderator = await client.users.fetch(ModeratorID)
					const expireUser =
						hasBlacklist.expire === 0
							? 'No Especificado'
							: `<t:${hasBlacklist.expire}:R>`
					const error = new MessageEmbed()
						...
						.addField(
							'`📆` | Fecha',
							`<t:${Math.floor(hasBlacklist.Date / 1000)}:R>`,
							true,
						)
						.addField('`🗑` | Expira', `${expireUser}`, true)
				...
				if (!hasBlacklist) {
					...
					const embed = new MessageEmbed()
						.setTitle('`💀` | black-list')
						.setDescription(
							`**${user.tag}** ha sido removido de la blacklist.`,
						)
						.setColor('#ff0000')
			...
			if (subcmd === 'add') {
				...
				if (!hasBlacklist) {
					...
					const embed = new MessageEmbed()
						.setTitle('`💀` | black-list')
						.setDescription(
							`**${server.name}** ha sido añadido a la blacklist.`,
						)
						.addField('`📑` | Razon', `\`${reason}\``, true)
						.addField(
							'`📆` | Fecha',
				...
				if (!hasBlacklist) {
					...
					const embed = new MessageEmbed()
						.setTitle('`💀` | black-list')
						.setDescription(
							`**${server.name}** ha sido removido de la blacklist.`,
						)
						.setColor('#ff0000')
