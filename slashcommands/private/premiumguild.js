		...
		.addSubcommand((o) =>
			o
				...
				.addStringOption((o) =>
					o
						.setName('tiempo')
						.setDescription('Tiempo de membresia.')
						.setRequired(false)
						.addChoices(
	...
	async run(client, interaction) {
		...
		if (subcmd === 'add') {
			const server = await client.guilds.fetch(
				interaction.options.getString('id-server'),
			)
			...
			if (
				!client.guilds.cache.has(`${interaction.options.getString('id-server')}`)
			) {
			...
			if (!premium) {
				...
				const year = now + 31536000
				const month = now + 2592000

				if (tiempo) {
					if (tiempo == '1mes') tiempo = ms('30d')
					else if (tiempo == '2meses') tiempo = ms('60d')
					else if (tiempo == '3meses') tiempo = ms('90d')
					else if (tiempo == '4meses') tiempo = ms('120d')
					else if (tiempo == '5meses') tiempo = ms('150d')
					else if (tiempo == '6meses') tiempo = ms('180d')
					else if (tiempo == '7meses') tiempo = ms('210d')
					else if (tiempo == '8meses') tiempo = ms('240d')
					else if (tiempo == '9meses') tiempo = ms('270d')
					else if (tiempo == '10meses') tiempo = ms('300d')
					else if (tiempo == '11meses') tiempo = ms('330d')
					else if (tiempo == '12meses') tiempo = ms('365d')
				...
				const pgembed = new MessageEmbed()
					.setTitle('✅Hecho')
				...
				const pgyet = new MessageEmbed()
					.setTitle('⚠ Advertencia')
					.setDescription(
						`El servidor \`${server.name}\` ya se encuentra en la lista!`,
					)
					.setColor('ORANGE')
		...
		if (subcmd === 'remove') {
			...
			if (!premium) {
				const pgno = new MessageEmbed()
					.setTitle('⚠ Advertencia')
					.setDescription(
						`El servidor \`${server.name}\` no esta registrado como premium`,
					)
					.setColor('ORANGE')
				...
				const pgyet = new MessageEmbed()
					.setTitle('✅ Hecho')
					.setDescription(
						`El servidor \`${server.name}\` ha sido eliminado de las membresias premium.`,
					)
					.setColor('GREEN')
