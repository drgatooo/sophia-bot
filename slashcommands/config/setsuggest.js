...
module.exports = {
    ...
    .addChannelOption(o => o.setName("canal").setDescription("En este canal se enviarán las sugerencias.").setRequired(true)),

    async run(client, interaction){
        const args = interaction.options;
        const channel = args.getChannel("canal");
        const results = await schema.findOne({guildid: interaction.guild.id});
        if(!channel.viewable) return interaction.reply({embeds: [
          new MessageEmbed()
          .setTitle(':x: Error')
        ...
        if(results) {
          ...
          let sendmsg = await interaction.reply({embeds: [
            new MessageEmbed()
          ...
          collector.on('collect', async(b) => {
              if(b.user.id !== interaction.user.id) return interaction.reply({content: "Solo el que puso el comando puede interactuar!", ephemeral: true});
