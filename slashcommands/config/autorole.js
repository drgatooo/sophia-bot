...
const command = {
    ...
    .addSubcommand(s =>
        s.setName('help')
        .setDescription("Muestra como setear los auto roles")
    ),
    ...
    async run(_, interaction){
        ...
        if(subcmd === "set") {
            ...
            const validateArrayData = rolesObj.every(obj => typeof obj.rol === "string" && typeof obj.nombreBoton === "string");
            const validateRoleEditable = rolesObj.every(obj => interaction.guild.roles.cache.get(obj.rol).editable === true);
            if(!validateArrayData) return await interaction.reply({ embeds: [
                new MessageEmbed()
                .setTitle(":x: Error")
                .setDescription("El formato de los datos no es correcto\nRevisa `/setautorole help` para saber cómo hacerlo")
                .setColor("RED")
            ], ephemeral: true});
            if(!validateRoleEditable) return await interaction.reply({ embeds: [
                new MessageEmbed()
                .setTitle(":x: Error")
                .setDescription("Alguno de los roles que agregaste no puedo manejarlo, por lo tanto no podre agregarlo.")
                .setColor("RED")
            ], ephemeral: true});
            ...
            const repeatedIds = componentsArr.map(obj => obj.customId).filter((value, index, self) => self.indexOf(value) !== index);
            if(repeatedIds.length > 0) return await interaction.reply({ embeds: [
                new MessageEmbed()
                .setTitle(":x: Error")
            ...
            if(results) {
                await schema.updateOne({ guildId: interaction.guild.id }, {
            ...
            await interaction.guild.channels.cache.get(channel.id).send({ embeds: [embed], components: [components] });
        ...
        await interaction.reply({ embeds: [
            new MessageEmbed()
            .setTitle(":x: Error")
            .setDescription("No se puedo transformar correctamente el JSON\nRevisa el comando `/setautorole help` para saber cómo hacerlo")
            .setColor("RED")
        ], ephemeral: true});
      }
    }
    ...
    if(subcmd === "help") {
        ...
        .setColor("GREEN")
        .setFooter({text: "Disfruta del bot!, Recuerda que solo puedes establecer un canal de autoroles.", iconURL: interaction.user.displayAvatarURL({ dynamic: true})})
        .setTimestamp();
