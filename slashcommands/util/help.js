...
module.exports = {
    ...
    async run(client, interaction){
		const emojis = {
            ...
            .setTimestamp()
            .setFooter(interaction.user.username, interaction.user.avatarURL({dynamic: true, format: 'png'}));
        ...
        .addComponents(menu);
        	...
        	const time = 60 * 1000;
        	const collector = interaction.channel.createMessageComponentCollector({filter: u => u.user.id === interaction.user.id, time});
        collector.on('collect',async i => {
            if(interaction.user.id !== i.user.id) return interaction.reply({content: 'No puedes interferir en el comando de otra persona.', ephemeral: true});
			categories.forEach(async (c) => {
                    if(i.values[0] === c && i.values[0] !== 'principal') {
                        await i.deferUpdate();
                        await interaction.editReply({embeds: [
                            new MessageEmbed()
                            .setTitle('comandos de '+c.toLowerCase()+' '+(emojis[c]||'➖'))
                            .setDescription(`\`${!interaction.channel.nsfw && c.toLowerCase() === 'nsfw' ? 'Debes estar en un canal nsfw para ver o usar estos comandos' : obj[c].map(cmd => cmd.name).join('` `')}\``)
                            .setColor('#00FFFF')
                            .setTimestamp()
                            .setFooter(interaction.user.username, interaction.user.avatarURL({format: 'png'}))
                        ], components: [row]});
                    }
                });
            ...
            setTimeout(async () => await interaction.editReply({ components: []}).catch(()=>{}), time);
    }
}