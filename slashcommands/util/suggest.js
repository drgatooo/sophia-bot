...
module.exports = {
    ...
    if(!channel || !interaction.guild.channels.cache.get(channel.channelid)) {
        err.setDescription("No hay ningun de sugerencias para este servidor, o ha sido eliminado. (para solucionarlo escribe: `/setsuggest`)")
    ...
    interaction.guild.channels.cache.get(channel.channelid).send({ embeds: [embedSuggest]}).then(msg => {
      msg.react('✅');
      msg.react('❌');
    });
