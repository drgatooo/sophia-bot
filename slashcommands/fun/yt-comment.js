...
const command = {
	...
	async run(_, interaction) {
		const mention = interaction.options.getUser('usuario')
			...
			: interaction.member // --define mention
		const comment = interaction.options.getString('texto') // --define comment
		const url = `${comment.replace(/ /g, '%20')}` // --define url
		if (url.length > 35) {
		...
		let result = `https://some-random-api.ml/canvas/youtube-comment?avatar=${mention.user.displayAvatarURL(
			{ format: 'png' },
		)}&comment=${url}&username=${username.replace(/ /g, '%20')}` // --define result
		result = new MessageAttachment(result, 'comment.png')
		await interaction.reply({
			embeds: [new MessageEmbed().setDescription('Listo! ❤').setColor('GREEN')],
			ephemeral: true,
		})
