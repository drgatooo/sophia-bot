...
const command = {
	...
	async run(client, interaction) {
		...
		const secondPlayer = args.getUser('usuario')

		if (!secondPlayer) {
			const arma = new MessageActionRow().addComponents(
				new MessageButton()
					.setCustomId('Pistola')
					.setStyle('SECONDARY')
					.setLabel('Dispara!')
			...
			setTimeout(
				async () =>
					await interaction.editReply({ embeds: [ya], components: [arma] }),
				3000,
			)
			...
			const Duelo = new MessageActionRow().addComponents(
				new MessageButton()
					.setCustomId('Pistola')
					.setStyle('SECONDARY')
					.setLabel('Dispara!')
			...
			const rechaza = new MessageEmbed()
				.setTitle('🤠 Lejano oeste')
			...
			const iFilter = (i) => i.user.id === secondPlayer.id
			...
			collector.on('collect', async (i) => {
				if (i.customId == 'Si') {
					...
					const inGame = (i) =>
						i.user.id === interaction.user.id || i.user.id === secondPlayer.id

					const inGameCollector =
					...
					inGameCollector.on('collect', async (i) => {
						if (i.customId == 'Pistola') {
							const ganador = new MessageEmbed()
								.setTitle('🤠 Lejano oeste')
								.setDescription(
									`💥 Bang!\n **Ha ganado: <@${i.user.id}> **!`,
								)
					...
					inGameCollector.on('end', (_collector, reason) => {
						if (reason === 'time') {
							const Nadie = new MessageEmbed()
								...
								.setDescription(
									'ℹ Se ve que nadie acciono el gatillo... todos pierden :)',
								)
								.setColor('WHITE')
