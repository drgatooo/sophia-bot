...
const command = {
	...
	async run(client, interaction) {
		...
		if (userCooldowned) {
			...
			const caras = 12
			const num = Math.floor(Math.random() * caras)

			const embed = new MessageEmbed()
				.setTitle(
					':game_die: :game_die: Has tirado los dados :game_die: :game_die:',
				)
				...
				if (num === 7) {
					...
					setTimeout(() => {
						interaction.editReply({
							embeds: [
								embed.addField(
									'Vaya, has sacado el mismo numero de posibilidad',
									`**${caras}**`,
									true,
								),
							],
					...
					setTimeout(() => {
						interaction.editReply({
							embeds: [
								embed.addField('Vaya, has sacado', `**${num}**`, true),
							],
						})
					}, 3000)
				}
