...
const command = {
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((o) =>
			o.setName('skin').setDescription('La skin que quieres ver').setRequired(true),
		),
	...
	async run(_, interaction) {
			...
			return await interaction.reply({
				embeds: [
					new MessageEmbed()
						.setTitle(':x: Error')
						.setDescription(
							'No puedes poner espacios en un nombre de minecraft!',
						)
						.setColor('RED'),
				],
				ephemeral: true,
		...
		if (require('../../helpers/hasEmoji.js')(args)) {
			return await interaction.reply({
				embeds: [
					new MessageEmbed()
						.setTitle(':x: Error')
						.setDescription(
							'No puedes poner emoji en un nombre de minecraft!',
						)
						.setColor('RED'),
				],
				ephemeral: true,
		...
		const embedSuccess = new MessageEmbed()
			.setDescription(
				'Skin de: `' +
					args +
					'` **(si es una skin innexistente saldra una por defecto)**',
			)
