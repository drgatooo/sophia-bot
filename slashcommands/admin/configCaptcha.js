...
module.exports = {
    ...
    async run(client, interaction) {
        if(!interaction.member.permissions.has("MANAGE_GUILD")) return interaction.reply({ embeds: [errorEmbed.setDescription(`Tienes que tener permisos para gestionar el servidor`)]}) //Damos error si la persona no tiene permisos para gestionar el servidor
...
async function enable(client, interaction, db) {
    ...
    if(db.enable == en) return interaction.reply({ embeds: [errorEmbed.setDescription(`La verificación ya está ${en == true ? `habilitada` : `deshabilitada`}`)], ephemeral: true}) //Si la opción es la misma que está en la base de datos avisamos que no es necesario cambiarla
    ...
    interaction.reply({ embeds: [successEmbed.setDescription(`Ahora el sistema de verificación captcha está ${en == true ? `habilitado` : `desactivado`}`)]}) //Avisamos que todo salió bien
}
...
async function role(client, interaction, db) {
    ...
    if(role) { //Si puso el parametro del rol
        if(!interaction.guild.roles.cache.has(role.id)) return interaction.reply({ embeds: [errorEmbed.setDescription(`El rol no pertenece a este servidor, por favor introduce un rol que se encuentre en el servidor`)], ephemeral: true}) //Si el rol no pertenece al servidor avisa
        if(interaction.guild.me.roles.highest.comparePositionTo(role) <= 0) return interaction.reply({ embeds: [errorEmbed.setDescription(`El rol es superior al mio`)], ephemeral: true}) //Si el rol es superior al del bot avisa sobre el fallo
        if(db.role && (db.role === role.id) == true){ //Si existe un rol en la db y es el mismo que la opción rol
            return interaction.reply({ embeds: [errorEmbed.setDescription(`El rol es el mismo que está almacenado en la base de datos`)], ephemeral: true}) //Damos aviso sobre ero
        } else { //Si no es el mismo rol
            db.role = `${role.id}` //Establecemos el nuevo rol
            interaction.reply({ embeds: [successEmbed.setDescription(`Ahora el rol **${role.name}** será dado a las personas que se verifiquen al usar el captcha`)]}) //Avisamos que todo salió bien
        }
    } else { //En caso de que no se diera la opción rol
        if((db.role == ``)) { //Si el rol en la base de datos está vacio
            return interaction.reply({ embeds: [errorEmbed.setDescription(`El rol ya estaba eliminado. Establece uno nuevo`)], ephemeral: true}) //Avisamos que ya está vacio
        } else { //Sino, se diera la opción del rol
            db.role = `` //El rol será removido de la base de datos
            interaction.reply({ embeds: [successEmbed.setDescription(`Se elimino el rol correctamente`)]}) //Mandamos aviso de que se eliminó el rol
        }
    }
...
async function channel(client, interaction, db) {
    ...
    if(channel) { //Si existe un canal
        if(!interaction.guild.channels.cache.has(channel.id)) return interaction.reply({ embeds: [errorEmbed.setDescription(`El canal no pertenece al servidor, por favor intenta con un canal que esté dentro del servidor`)], ephemeral: true}) //Si el canal no se encuentrá en el servidor avisamos
        if(!channel.permissionsFor(interaction.guild.me).has("VIEW_CHANNEL")) return interaction.reply({ embeds: [errorEmbed.setDescription(`No puedo ver el canal`)], ephemeral: true}) //Si el bot no puede leer mensajes en el canal avisará
        if(channel.type !== `GUILD_TEXT`) return interaction.reply({ embeds: [errorEmbed.setDescription(`El canal debe de ser un canal de texto normal`)], ephemeral: true}) //Si el tipo del canal no es de tipo GUILD_TEXT avisamos que no se puede establecer ese canal
        if(db.channel && (db.channel === channel.id) == true) { //Si existe un canal en la db y es el mismo que está en la db
            return interaction.reply({ embeds: [errorEmbed.setDescription(`El canal es el mismo que se encuentra en la base de datos`)], ephemeral: true}) //Avisamos que no es necesario cambiar el canal
        } else { //Si no existe o no es el mismo
            db.channel = `${channel.id}` //Guardamos el nuevo canal
            interaction.reply({ embeds: [successEmbed.setDescription("El comando `/captcha` ahora solo se puede usar en el canal <#" + channel.id + ">")]}) //Mandamos un mensaje de que se cambió el canal
        }
    } else { //En caso de que no se tenga la opción del canal
        if(db.channel == ``) { //Si no existe canal
            return interaction.reply({ embeds: [errorEmbed.setDescription(`No existe un canal en la base de datos, por lo que no se puede borrar`)], ephemeral: true}) //Avisamos que ya está eliminado y no es necesario usarlo
